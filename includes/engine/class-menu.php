	...
	private function getMenuItemTree(int $id, array $items, bool $is_top_level): array {
		...
		foreach($itemmeta as $meta) {
			...
			if(isset($meta['post_link'])) {
				...
				if(!empty($type) && array_key_exists($type, $rs_post_types) && $rs_post_types[$type]['show_in_nav_menus']) {
				...
				$tax = $rs_query->selectField(getTable('ta'), 'name', array(
					'id' => $tax_id
				));
				
				if(!empty($tax) && array_key_exists($tax, $rs_taxonomies) && $rs_taxonomies[$tax]['show_in_nav_menus'])
					$permalink = getPermalink($tax, $this->getMenuItemParent($meta['term_link']));
