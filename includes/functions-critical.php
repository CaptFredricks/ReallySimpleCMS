...
requireFile(PATH . UTILS . '/polyfills.php');
 ...
 * @var object
 */
$rs_ad_register = new \Admin\Register;
...
function requireFile(string $filepath, bool $once = true): void {
	...
	global $rs_query, $rs_session, $rs_register, $rs_ad_register, $rs_error, $rs_theme_path,
		$rs_post_types, $rs_taxonomies, $rs_modules, $rs_themes, $rs_admin_themes;
...
function baseSetup(): void {
	...
	runAdminPagesRegister(); // Register admin pages
	runPostTypesRegister(); // Register post types
	runTaxonomiesRegister(); // Register taxonomies
	runModulesRegister(); // Register modules
	runThemesRegister(); // Register themes
	runAdminThemesRegister(); // Register admin themes
	
	// Maintenance mode, triggered by defining `MAINT_MODE` in `config.php`
	if((defined('MAINT_MODE') && MAINT_MODE === true) && (!isLogin() && !is404()) && empty($rs_session))
		requireFile(PATH . UTILS . '/maintenance.php');
	
	// An update is running; triggered by the update system
	#if(isset($rs_doing_upgrade) && $rs_doing_upgrade === true)
	if(!defined('BASE_INIT') || (defined('BASE_INIT') && BASE_INIT === false) && !empty($rs_session))
		requireFile(PATH . UTILS . '/update.php');
}
...
function themeSetup(): void {
	...
	if(!defined('BASE_INIT') || (defined('BASE_INIT') && BASE_INIT === false)) {
		...
		if(isLogin()) {
			...
			requireFiles(array(
				RS_THEME_FUNC, // Theme-specific functions
			...
			requireFiles(array(
				PATH . UTILS . '/sitemaps.php', // Sitemap loader
...
/**
 * Make sure the server is running the required PHP version and Apache modules.
 * @since 1.3.9-beta
 */
function checkPHPSetup(): void {
	$notice = array();
	...
	if(function_exists('apache_get_modules')) {
		...
		// Falls back to checking server variables if apache_get_modules is unavailable
		$rewrite_enabled = (isset($_SERVER['HTTP_MOD_REWRITE']) && $_SERVER['HTTP_MOD_REWRITE'] == 'On');
	}
	
	if(!$rewrite_enabled) {
		$notice[] = 'The rewrite module is required to run ' . RS_ENGINE . '. You can activate it in your PHP server settings or configuration file.';
	}
	
	if(!empty($notice))
		exit('<p>' . implode('<br><br>', $notice) . '</p>');
}
