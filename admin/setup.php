...
if(file_exists(PATH.INC.'/config.php')) exit('A config file already exists. If you wish to continue installation, please delete the existing file.');
...
<html>
	<head>
		<title>Really Simple CMS Setup</title>
		<meta charset="UTF-8">
...
switch($step) {
	case 0:
		?>
		<p>Welcome to the Really Simple CMS! To install the CMS and start building your site, you will need to provide the following information. All of this can be obtained from your hosting provider.</p>
		<ol>
			<li>Database name</li>
	...
	case 2:
		...
		if(!$rs_query->conn_status) {
			?>
			<p><strong>Error!</strong> Really Simple CMS could not connect to the database. Please return to the previous page and make sure all the provided information is correct.</p>
		...
		if(!is_writable(PATH)) {
			...
			<p>If that's the case, just copy the code below and create <code>config.php</code> in the <code>includes</code> directory of Really Simple CMS.</p>
			<?php
			$text = '';
